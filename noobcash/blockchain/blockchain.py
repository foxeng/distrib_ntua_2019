...
def _check_for_new_block() -> None:
    ...
    new_block_tx = []
    for t in tx_pool:
        if all(r.hexists(utxo_block, i.dumpb()) for i in t.inputs):
            new_block_tx.append(t)
            if len(new_block_tx) == CAPACITY:
                new_block = Block(index=last_block.index + 1,
                                  previous_hash=last_block.current_hash,
                                  transactions=new_block_tx)
                r.hdel("blockchain:tx_pool", *(t.id for t in new_block_tx))
...
def new_recv_transaction(t: Transaction) -> bool:
    ...
    # TODO OPT: Turn this into a Lua script to use with redis EVAL
...
def new_recv_block(recv_block: Block) -> bool:
    ...
    if recv_block.is_genesis():
        ...
        ob = o.dumpb()
        r.hset("blockchain:utxo-block", ib, ob)
    ...
    if recv_block.previous_hash == last_block.current_hash:
        ...
        for t in recv_block.transactions:
            ...
            input_amount = 0.0
            for i in t.inputs:
                o = r.hget("blockchain:utxo-block:".encode() + last_block.current_hash, i.dumpb())
                if o is None:
                    return False
                referenced_txos.add(i.dumpb())
        ...
        utxo_last_block = r.dump("blockchain:utxo-block:".encode() + last_block.current_hash)
        r.restore("blockchain:utxo-block:".encode() + recv_block.current_hash, 0, utxo_last_block,
                  replace=True)
        ...
        #       along the way
        old_main_branch = []    # the Blocks in the old main branch
        ...
        #       to the main branch:
        osb_referenced_txos = []    # referenced_txos for each block in the old side branch
        for b in old_side_branch:
            ...
            for t in b.transactions:
                ...
                input_amount = 0
                for i in t.inputs:
                    o = r.hget("blockchain:utxo-block:".encode() + b.previous_hash, i.dumpb())
                    if o is None:
            ...
            osb_referenced_txos.append(referenced_txos)
        ...
        #       with block
        #       TODO: Taken care of with a transaction rollback?
        ...
        for b in reversed(old_main_branch):
            ...
            for t in b.transactions:
                ...
                #       transactions the pool as it is at this point (current as of the old main
        ...
        conflicting_tx = set()
        for b, referenced_txos in zip(old_side_branch, osb_referenced_txos):
